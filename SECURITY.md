...
Database accounts support standards-compatible six-digit TOTP MFA with a one-step clock window. Enrollment requires password re-authentication, remains pending until a valid TOTP is supplied, and stores its server-generated secret under versioned AES-256-GCM authenticated encryption. `MFA_ENCRYPTION_KEY` is a dedicated base64url 32-byte deployment secret and must not reuse `SESSION_SECRET`; production fails closed when it is absent or invalid. Recovery codes are revealed only upon activation/regeneration, stored only as individual strong hashes, and atomically consumed once.
