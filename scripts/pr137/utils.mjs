...
export async function replaceExact(path, from, to) {
  ...
  if (first < 0) throw new Error(`Missing replacement target in ${path}: ${from.slice(0, 120)}`);
  if (source.indexOf(from, first + from.length) >= 0)
    throw new Error(`Replacement target is not unique in ${path}: ${from.slice(0, 120)}`);
