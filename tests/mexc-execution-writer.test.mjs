...
const intent={userId:"user-1",accountId:"account-1",intentId:"intent-id-315",idempotencyKey:"intent-315",symbol:"BTC_USDT",side:"long",orderType:"limit",positionMode:"one-way",positionId:"991",marginMode:"isolated",positionVolume:3,volume:1,price:65000,referencePrice:65000,estimatedNotional:65,leverage:1,reduceOnly:true,bindingGeneration:"binding-1",writeCredentialGeneration:"generation-1",rolloutRevision:2,riskRevision:3,reconciliationRevision:4};
...
test("pre-write authority is derived from kill switches, opposing position evidence, and the exact prepared limit preview",()=>{const authority=composeMexcPreWriteAuthority(intent,authorityEvidence,1700000000000);assert.ok(Object.values(authority).every(Boolean));for(const changed of [{account:true},{maintenance:true},{emergencyStop:true},{armed:false},{providerStateFresh:false}]){const next={...switches};if(changed.account)next.disabledAccountKeys=new Set([JSON.stringify([intent.userId,intent.accountId])]);else Object.assign(next,changed);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,switches:next},1700000000000).killSwitchesClear,false);}assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,airlock:{...authorityEvidence.airlock,result:{...prepared,preview:{...prepared.preview,normalizedContractVolume:2}}}},1700000000000).airlockPrepared,false);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,airlock:{...authorityEvidence.airlock,result:{...prepared,preview:{...prepared.preview,orderType:"market"}}}},1700000000000).airlockPrepared,false);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,reconciliation:{...authorityEvidence.reconciliation,positionSide:"long"}},1700000000000).reconciliationClean,false);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,reconciliation:{...authorityEvidence.reconciliation,marginMode:"cross"}},1700000000000).reconciliationClean,false);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,network:{...authorityEvidence.network,writeCredentialGeneration:"generation-2"}},1700000000000).networkAllowlisted,false);assert.equal(composeMexcPreWriteAuthority(intent,{...authorityEvidence,network:{mexcEgressAllowlisted:true}},1700000000000).networkAllowlisted,false);});

test("short/sell reduction serializes close-long side 4",async()=>{const calls=[],store=new SqliteMexcExecutionLifecycleStore(":memory:"),shortIntent={...intent,idempotencyKey:"short-reduce",side:"short"},writer=new ModernMexcReduceOnlyWriter(async request=>{calls.push(request);return response("short-1",{externalOid:mexcExternalOid(shortIntent),side:4})},store,()=>1700000000000,0);await execute(writer,shortIntent);assert.equal(JSON.parse(calls[0].body).side,4);store.close();});

test("stable externalOid quarantines authority revision and generation rotations before POST",async()=>{for(const changed of [{rolloutRevision:20},{riskRevision:30},{reconciliationRevision:40},{bindingGeneration:"binding-2"},{writeCredentialGeneration:"generation-2"}]){let posts=0;const store=new SqliteMexcExecutionLifecycleStore(":memory:"),writer=new ModernMexcReduceOnlyWriter(async request=>{if(request.method==="POST")posts++;return response("one")},store,()=>1700000000000,0);await execute(writer);const rotated={...intent,...changed};assert.equal(mexcExternalOid(rotated),mexcExternalOid(intent));const rotatedCredentials=changed.writeCredentialGeneration?{...credentials,generation:changed.writeCredentialGeneration}:credentials;await assert.rejects(execute(writer,rotated,rotatedCredentials),error=>error.kind==="quarantined");assert.equal(posts,1);store.close();}});
...
test("queued requests obtain fresh authority and credential generation after the writer wait",async t=>{
  const cases={
    ...
    "egress attestation":state=>state.evidence={...state.evidence,network:{...state.evidence.network,mexcEgressAllowlisted:false}},
    "egress credential generation":state=>state.evidence={...state.evidence,network:{...state.evidence.network,writeCredentialGeneration:"generation-2"}},
