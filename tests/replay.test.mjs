...
import {clampReplayCursor,createReplaySession,createReplaySnapshot,detectReplayGaps,jumpReplay,jumpReplayToTimestamp,prepareReplayCandles,progressReplay,replayCursorAtOrBefore,replayDelayMs,replayIdentityChanged,replayPrefix,replayRangeForCandles,ReplayRequestGate,stepReplay} from "../app/lib/replay.ts";
import {DEFAULT_RISK,DEFAULT_STRATEGY} from "../app/lib/config.ts";
const candle=(time,close=time)=>({time,open:close,high:close+1,low:close-1,close,volume:1});
const candles=[candle(60),candle(120),candle(240)];
const session=(items=candles)=>createReplaySession({id:"r1",symbol:"BTC_USDT",timeframe:"1m",rangeStartMs:60000,rangeEndMs:240000,startedAt:1,candles:items});
...
test("clamps, steps, jumps and ends deterministically",()=>{assert.equal(clampReplayCursor(99,3),2);assert.equal(stepReplay(session(),candles,1).cursorIndex,1);assert.equal(stepReplay(session(),candles,-1).cursorIndex,0);assert.equal(jumpReplay(session(),candles,99).status,"ended");assert.equal(jumpReplayToTimestamp(session(),candles,121000).cursorIndex,1);assert.equal(progressReplay({...session(),status:"playing"},candles).cursorIndex,1);});
test("timestamp selection never advances into a future candle",()=>{assert.equal(replayCursorAtOrBefore(candles,59999),null);assert.equal(replayCursorAtOrBefore(candles,60000),0);assert.equal(replayCursorAtOrBefore(candles,121000),1);assert.equal(replayCursorAtOrBefore(candles,239999),1);assert.equal(replayCursorAtOrBefore(candles,999999),2);});
