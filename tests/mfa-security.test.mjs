...
test("execution-grade resolution requires TOTP session assurance and live MFA",()=>fixture(async user=>{beginMfaEnrollment(user.id);confirmMfaEnrollment(user.id,code(secret()),now);const password=createDatabaseSession(user,3600),recovery=createDatabaseSession(user,3600,"recovery"),totpToken=createDatabaseSession(user,3600,"totp");assert.equal(executionGradeDatabaseSession(password),null);assert.equal(executionGradeDatabaseSession(recovery),null);assert.equal(executionGradeDatabaseSession(createSessionToken(VIEWER_USER)),null);const grade=executionGradeDatabaseSession(totpToken);assert.equal(grade.userId,user.id);assert.equal(executionGradeDatabaseSessionFingerprint(grade.sessionFingerprint).userId,user.id);disableMfa(user.id);assert.equal(executionGradeDatabaseSession(totpToken),null);assert.equal(executionGradeDatabaseSessionFingerprint(grade.sessionFingerprint),null)}));
