...
test("valid older armed backing replacement permanently poisons the store",()=>{const dir=directory(),path=join(dir,"rollout.sqlite"),old=join(dir,"old-armed.sqlite"),replacement=join(dir,"replacement.sqlite");try{let store=new SqliteExecutionRolloutStore(path);const approved=store.approve(id,digest,1,policy,at,0);const armed=store.arm(id,at,approved.revision);store.close();copyFileSync(path,old);store=new SqliteExecutionRolloutStore(path);const stopped=store.disarm(id,at,armed.revision);assert.equal(stopped.status,"disarmed");copyFileSync(old,replacement);renameSync(replacement,path);const unavailable=e=>e instanceof ExecutionRolloutStoreError&&e.code==="EXECUTION_ROLLOUT_UNAVAILABLE";assert.throws(()=>store.read(id),unavailable);assert.throws(()=>store.read(id),unavailable);assert.throws(()=>store.events(id),unavailable);assert.throws(()=>store.arm(id,at,armed.revision),unavailable);assert.throws(()=>store.revoke(id,at,stopped.revision),unavailable)}finally{rmSync(dir,{recursive:true,force:true})}});
