...
const modulePath = "app/lib/mexc-owner-account-snapshot.ts";
...
test("owner snapshot service delegates to the reviewed GET-only transport", () => {
  const source = text(modulePath);
  assert.match(source, /^import "server-only";/);
  assert.match(source, /requestMexcPrivateRead/);
  assert.match(source, /endpoint:\s*request\.endpoint/);
...
test("owner snapshot service is not exposed by an API route or client component", () => {
  const modulePattern = /mexc-owner-account-snapshot/;
  const apiFiles = filesBelow("app/api").filter((path) => /\.[cm]?[jt]sx?$/.test(path));
  for (const path of apiFiles) {
    assert.doesNotMatch(text(path), modulePattern, path);
  }
