...
test("production ownership gate is ahead of reconciliation and provider evaluation", () => {
  const composition = text("app/lib/execution/internal/composition.ts");
  const killSwitch = composition.indexOf("executionKillSwitchReason(");
  const proveOwnership = composition.indexOf("await proveOwnership(caller)");
  const ownershipRead = composition.indexOf("ownershipStore.read(caller)");
  const reconcile = composition.indexOf("await reconcile(");
  assert.ok(
    killSwitch >= 0 &&
      proveOwnership > killSwitch &&
      ownershipRead > proveOwnership &&
      reconcile > ownershipRead,
  );
  const boundary = text("app/lib/execution/internal/boundary-service.ts");
  assert.ok(
    boundary.indexOf("executionOwnershipStore") <
      boundary.indexOf("executionReconciliationStore"),
  );
...
test("ownership ceremony is server-only and has no public route or credential custody wiring", () => {
  for (const path of [
    ...
    assert.doesNotMatch(
      source,
      /credential-custody|apiSecret|apiKey|decrypt|CREDENTIAL_CUSTODY|LIVE_TRADING_ENABLED/,
    );
  }
  assert.doesNotMatch(
  ...
  const routeSources = readdirSync("app/api", { recursive: true })
    .filter((path) => String(path).endsWith("route.ts"))
    .map((path) => text(`app/api/${path}`))
    .join("\n");
  assert.doesNotMatch(routeSources, /execution-ownership|ownership-ceremony/);
});
test("MEXC Radar remains fixed-path GET-only and no executable adapter is introduced", () => {
  ...
  assert.doesNotMatch(
    text("app/lib/execution/internal/adapter.ts"),
    /executed:\s*true/,
  );
  assert.doesNotMatch(text("app/lib/execution/types.ts"), /executed:\s*true/);
  const executionSources = readdirSync("app/lib/execution", { recursive: true })
    .filter((path) => String(path).endsWith(".ts"))
    .map((path) => text(`app/lib/execution/${path}`))
    .join("\n");
  assert.doesNotMatch(executionSources, /executed:\s*true/);
  assert.doesNotMatch(executionSources, /method:\s*"(POST|PUT|PATCH|DELETE)"/);
});
test("ownership persistence contains only bounded identity/state/timestamps and secret-free events", () => {
  ...
  assert.doesNotMatch(
    store,
    /api.?key|api.?secret|credential|callerAssertion|sessionToken/i,
  );
});
