...
test("parses public MEXC contract leverage and precision", () => {
  ...
  assert.equal(contract.maxLeverage, 1000);
  assert.equal(contract.contractSize, 0.001);
  assert.equal(contract.maintenanceMarginRate, 0.0004);
  assert.equal(contract.positionOpenType, 3);
});

test("builds practical stops while preserving and enforcing range endpoints", () => {
  const contract = parseMexcContractMetadata(xauPayload, "XAU_USDT");
  assert.deepEqual(leverageStopsForContract(contract), [
    1, 2, 3, 5, 10, 20, 25, 50, 75, 100, 125, 200, 500, 1000,
  ]);
  assert.equal(clampContractLeverage(-50, contract), 1);
  assert.equal(clampContractLeverage(Number.NaN, contract), 1);
  assert.equal(clampContractLeverage(999.6, contract), 1000);
  assert.equal(clampContractLeverage(5000, contract), 1000);
});
...
test("Manual Paper ticket requires contract rules and previews the contract maintenance rate", () => {
  ...
  assert.match(
    source,
    /!publicPrice \|\|\s*!contract \|\|\s*invalidAmount/,
  );
  assert.match(
    source,
