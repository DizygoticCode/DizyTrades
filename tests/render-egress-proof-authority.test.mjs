...
const ips=Object.freeze(["9.9.9.9","1.1.1.1","8.8.8.8"]), canonical=Object.freeze(["1.1.1.1","8.8.8.8","9.9.9.9"]);
...
test("dedicated IPv4 proof is exactly three distinct canonical public addresses",()=>{
  assert.deepEqual(canonicalDedicatedIpv4s(ips),canonical);
  assert.equal(renderDedicatedIpSetDigestSha256(ips),renderDedicatedIpSetDigestSha256(canonical));
  assert.equal(renderDedicatedIpSetDigestSha256(["1.1.1.1","1.1.1.1","8.8.8.8"]),null);
  assert.equal(renderDedicatedIpSetDigestSha256(["10.0.0.1","1.1.1.1","8.8.8.8"]),null);
...
test("egress proof is exact-generation, CAS revisioned, twice-observed, allowlisted and sticky-revoked",()=>{
  const s=new SqliteRenderEgressProofStore(":memory:");try{
    ...
    assert.throws(()=>s.observe(identity,serviceId,"8.8.8.8","b".repeat(40),"instance-2",new Date(Date.parse(at(1))+30_000).toISOString(),2),conflict);
    state=s.observe(identity,serviceId,"8.8.8.8","b".repeat(40),"instance-2",at(2),2);assert.equal(state.observationCount,2);
    ...
    state=s.observe(identity,serviceId,"9.9.9.9","c".repeat(40),"instance-3",at(3),4);assert.equal(state.status,"allowlisted");assert.equal(state.observationCount,3);
...
test("proof rejects undeclared service/IP, stale allowlisting and private addresses",()=>{
  const s=new SqliteRenderEgressProofStore(":memory:");try{
    ...
    state=s.observe(identity,serviceId,"8.8.8.8","b".repeat(40),"i2",at(2),2);
    assert.throws(()=>s.allowlist(identity,state.ipSetDigestSha256,new Date(Date.parse(at(2))+10*60_000+1).toISOString(),3),conflict);
    assert.throws(()=>s.declare(id("bad"),serviceId,"oregon",["10.0.0.1","1.1.1.1","8.8.8.8"],at(0),0),invalid);
  }finally{s.close();}
});
...
test("same generation is isolated by exact account identity and survives restart",()=>{
  const root=mkdtempSync(join(tmpdir(),"render-egress-proof-")),path=join(root,"proof.sqlite");try{
    ...
    s.declare(other,"srv-bbbbbbbbbbbbbbbbbbbb","ohio",["4.2.2.1","4.2.2.2","4.2.2.3"],at(0),0);s.close();
...
test("mutations require owner DB session + current password + fresh non-replayable TOTP",async()=>{
  ...
  const store=new SqliteRenderEgressProofStore(join(root,"proof.sqlite"));try{
    ...
    const t3=t2+60_000,second=await observeRenderDedicatedEgress(store,req(t3,2),{...runtime,gitCommit:"b".repeat(40),instanceId:"instance-2"},"8.8.8.8",new Date(t3));assert.equal(second?.observationCount,2);
