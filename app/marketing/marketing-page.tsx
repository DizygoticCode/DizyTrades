...
export default function MarketingPage() {
  return <div className="marketing-shell"><SiteHeader /><main>
    <section className="hero"><div className="hero-glow" aria-hidden="true" /><div className="eyebrow"><i /> EVERYTHING DIZY™</div><h1>See the market.<br /><span>Understand the decision.</span></h1><p>DizyTrades connects market discovery, charting, confirmed-candle analysis, order flow, futures simulation, Replay, structured review, realised performance, education and recovery in one transparent workspace.</p><div className="hero-actions"><Link className="button primary" href="/explore">Open View-Only Terminal <span aria-hidden="true">→</span></Link><Link className="button secondary" href="/school">Explore {SCHOOL_DISPLAY_NAME}</Link><Link className="text-action" href="/signup">Create Account</Link><Link className="text-action" href="/login">Sign In</Link></div><div className="trust-row"><span><i /> Public market data</span><span><i /> Closed-candle discipline</span><span><i /> Evidence-first review</span><span><i /> Live trading disabled</span></div></section>
    <section className="preview-section" aria-labelledby="preview-title"><div className="section-heading"><div><span>THE WORKSPACE</span><h2 id="preview-title">One terminal.<br />One evidence chain.</h2></div><p>Move from market discovery and structure to signals, flow, simulation, historical reconstruction, review and measurement without losing the original context.</p></div><TerminalPreview /><p className="preview-note">Efficient illustrative preview — no live feed is loaded on this page.</p></section>
    ...
    <section className="safety section" aria-labelledby="safety-title"><div><div className="section-kicker">SECURITY BY BOUNDARY</div><h2 id="safety-title">Research freely.<br />Risk nothing real.</h2></div><div className="safety-grid"><article><b>LIVE TRADING</b><strong>Disabled</strong><p>DizyTrades is an active beta. There is no enabled exchange order route and no live execution.</p></article><article><b>CREDENTIALS</b><strong>Never requested</strong><p>Current market, simulation and viewer workflows use public data. Exchange credentials do not belong in the browser.</p></article><article><b>RECOVERY</b><strong>Owner controlled</strong><p>DizyBackup excludes credentials, requires a validated dry-run and applies only conflict-aware additive recovery.</p></article></div><p className="safety-callout"><i /> DizyTrade exchange connectivity is <b>Coming Later</b> and requires read-only shadow mode plus a separate security review.</p></section>
