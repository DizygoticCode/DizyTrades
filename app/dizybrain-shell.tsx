...
export type DizyBrainDepthSummary = Readonly<{ visible: boolean; bidTotal: number; askTotal: number; clusters: number; scaling: string; displayMode: string }>;

type Controller = {
  preferences: DizyBrainWorkspacePreferences;
  data: DizyBrainWorkspaceData | null;
  launcherRef: React.RefObject<HTMLButtonElement | null>;
  depth: DizyBrainDepthSummary | null;
  publish(data: DizyBrainWorkspaceData): void;
  publishDepth(data: DizyBrainDepthSummary): void;
...
export function DizyBrainShell({ children }: { children: ReactNode }) {
  ...
  const [depth, setDepth] = useState<DizyBrainDepthSummary | null>(null);
  ...
  const publishDepth = useCallback((next: DizyBrainDepthSummary) => setDepth(next), []);
  const controller = useMemo(() => ({ preferences, data, depth, launcherRef, publish, publishDepth, open, close, update }), [preferences, data, depth, publish, publishDepth, open, close, update]);
...
function ModuleBody({ data, depth, module }: { data: DizyBrainWorkspaceData | null; depth: DizyBrainDepthSummary | null; module: DizyBrainWorkspaceModule }) {
  ...
  if (module === "flow") {
    ...
    return <><Disclosure title="Summary" open><Row label="Feed" value={flow.availability} /><Row label="Evidence confidence" value={`${flow.intelligenceConfidence}% · ${flow.confidenceBand}`} /><Row label="Reference" value={`${flow.referencePriceSource} · ${flow.referencePrice}`} /><Row label="Spread" value={flow.spread.percentage === null ? "Unavailable" : `${flow.spread.percentage.toFixed(4)}% · ${flow.spread.classification}`} /></Disclosure>
      <Disclosure title="Liquidity"><Row label="Visible wall candidates" value={flow.walls.candidates.length} /><Row label="Withdrawals / replenishments" value={`${flow.walls.withdrawals.length} / ${flow.walls.replenishment.length}`} />{depth?.visible ? <><Row label="Market Depth bid / ask" value={`${depth.bidTotal.toLocaleString()} / ${depth.askTotal.toLocaleString()}`} /><Row label="Depth imbalance" value={`${depth.bidTotal + depth.askTotal ? ((depth.bidTotal-depth.askTotal)/(depth.bidTotal+depth.askTotal)*100).toFixed(1) : "0.0"}%`} /><Row label="Large clusters" value={depth.clusters} /><Row label="Presentation" value={`${depth.scaling} · ${depth.displayMode}`} /><p className="brain-note">Resting orders can be cancelled, moved or consumed and do not predict future price.</p></> : <Row label="Market Depth visual" value="Off" />}<div className="brain-table"><table><thead><tr><th>Band</th><th>Bid</th><th>Ask</th><th>Imbalance</th></tr></thead><tbody>{flow.depth.bands.map((band, index) => <tr key={band.bandPct}><th>{band.bandPct}%</th><td>{band.bidNotional.toLocaleString()}</td><td>{band.askNotional.toLocaleString()}</td><td>{flow.imbalance.bands[index].value === null ? "—" : `${(flow.imbalance.bands[index].value! * 100).toFixed(1)}%`}</td></tr>)}</tbody></table></div></Disclosure>
  ...
  return <><Disclosure title="Terminal" open><Row label="Market" value={`${data.market} · ${data.symbol}`} /><Row label="Chart feed" value={data.feedState} /><Row label="Replay evidence" value={data.replay ? "Historical source active; current feed is not Replay evidence" : "Idle"} /><Row label="Viewer mode" value={data.viewer ? "Read only" : "Owner"} /></Disclosure><Disclosure title="Current live DizyFlow feed (not historical evidence)">{data.replay ? <p className="brain-note">These versions describe the current live feed only and do not describe the historical Replay point.</p> : null}<Row label="Engine" value={flow?.engineVersion ?? "Unavailable"} /><Row label="Schema" value={flow?.schemaVersion ?? "Unavailable"} /><Row label="Config" value={flow?.configVersion ?? "Unavailable"} /></Disclosure></>;
}

export function DizyBrainWorkspace() {
  const { preferences, data, depth, open, close, update } = useDizyBrainWorkspace();
  ...
  return <><button className={`brain-mobile-backdrop ${overlay ? "visible" : ""}`} aria-label="Close DizyBrain workspace" onClick={close} type="button" /><aside id="dizybrain-workspace" ref={panelRef} className={`dizybrain-workspace ${overlay ? "drawer" : ""}`} style={{ "--brain-width": `${preferences.width}px` } as React.CSSProperties} aria-label="DizyBrain Analysis Workspace">
    ...
    <section className="brain-module" aria-labelledby="brain-module-title"><h2 id="brain-module-title">{DIZYBRAIN_MODULES.find(item => item.id === preferences.selectedModule)?.label}</h2><ModuleBody data={data} depth={depth} module={preferences.selectedModule} /></section>
  </aside></>;
}
