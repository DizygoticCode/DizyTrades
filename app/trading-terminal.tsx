            ...
            {settingsOpen && user.role !== "viewer" ? (
              ...
              >
                ...
                <option value="pine-v1-exact">Pine V1 Exact</option>
                  ...
                  {activePanel === "dizyflow" ? <div className="setting-section flow-settings"><h3>DizyFlow · public data</h3><p className="setting-help">Bounded browser-memory depth and executed-trade rendering. Capture begins only when enabled.</p>{[["enabled","Master capture"],["heatmapVisible","Heatmap"],["bubblesVisible","Volume bubbles"],["domVisible","Depth of Market"],["alertsVisible","Large-activity alerts"],["imbalanceVisible","Imbalance"]].map(([key,label])=><IndicatorToggle key={key} checked={orderFlowSettings[key as keyof OrderFlowSettings] as boolean} label={label} colour="#9c78ff" onChange={checked=>setOrderFlowSettings(current=>({...current,[key]:checked}))}/>)}<h3>Heatmap</h3><label className="field-row"><span>Colour map</span><select value={orderFlowSettings.heatmap.colourMap} onChange={()=>{}}><option value="bookmap">Bookmap inspired</option></select></label><RangeField label="Overall opacity" min={0} max={100} suffix="%" value={Math.round(orderFlowSettings.heatmap.opacity*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,opacity:value/100}}))}/><RangeField label="Brightness" min={25} max={300} suffix="%" value={Math.round(orderFlowSettings.heatmap.brightness*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,brightness:value/100}}))}/><RangeField label="Contrast" min={25} max={300} suffix="%" value={Math.round(orderFlowSettings.heatmap.contrast*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,contrast:value/100}}))}/><RangeField label="Gamma" min={20} max={300} suffix="%" value={Math.round(orderFlowSettings.heatmap.gamma*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,gamma:value/100}}))}/><RangeField label="Lower cut-off" min={0} max={49} suffix="%" value={Math.round(orderFlowSettings.heatmap.lowerPercentile*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,lowerPercentile:value/100}}))}/><RangeField label="Upper cut-off" min={51} max={100} suffix="%" value={Math.round(orderFlowSettings.heatmap.upperPercentile*100)} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,upperPercentile:value/100}}))}/><label className="field-row"><span>Intensity</span><select value={orderFlowSettings.heatmap.intensity} onChange={e=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,intensity:e.target.value as "log"|"linear"}}))}><option value="log">Logarithmic</option><option value="linear">Linear</option></select></label><label className="field-row"><span>Vertical smoothing</span><select value={orderFlowSettings.heatmap.priceMode} onChange={e=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,priceMode:e.target.value as "auto"|"manual"|"none"}}))}><option value="auto">Auto</option><option value="manual">Manual</option><option value="none">None</option></select></label><label className="field-row"><span>Manual price-bin size</span><input type="number" min="0.00000001" step="any" value={orderFlowSettings.heatmap.fixedPriceStep} onChange={e=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,fixedPriceStep:Number(e.target.value)}}))}/></label><RangeField label="History duration" min={5} max={240} suffix="min" value={orderFlowSettings.heatmap.historyMinutes} onChange={value=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,historyMinutes:value}}))}/><label className="field-row"><span>Book side</span><select value={orderFlowSettings.heatmap.side} onChange={e=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,side:e.target.value as "both"|"bids"|"asks"}}))}><option value="both">Both</option><option value="bids">Bid only</option><option value="asks">Ask only</option></select></label><label className="field-row"><span>Minimum order notional</span><input type="number" min="0" value={orderFlowSettings.heatmap.minimumNotional} onChange={e=>setOrderFlowSettings(v=>({...v,heatmap:{...v.heatmap,minimumNotional:Number(e.target.value)}}))}/></label><h3>Bubbles & alerts</h3><RangeField label="Minimum bubble notional" min={0} max={1000000} suffix="USDT" value={orderFlowSettings.bubbles.minimumNotional} onChange={value=>setOrderFlowSettings(v=>({...v,bubbles:{...v.bubbles,minimumNotional:value}}))}/><RangeField label="Large trade threshold" min={0} max={10000000} suffix="USDT" value={orderFlowSettings.alerts.fixedThreshold} onChange={value=>setOrderFlowSettings(v=>({...v,alerts:{...v.alerts,fixedThreshold:value}}))}/><button className="secondary" type="button" onClick={orderFlow.clear}>Clear captured data</button></div> : null}
                </div>
