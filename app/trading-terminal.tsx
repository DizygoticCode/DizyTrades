...
function drawChartOverlay(canvas: HTMLCanvasElement, chart: IChartApi, candleSeries: ISeriesApi<"Candlestick">, candles: Candle[], analysis: StrategyAnalysis, view: ViewSettings) {
  ...
  if (view.supportResistance) {
    ...
    drawable.forEach((item) => {
      ...
      const extent = calculateHorizontalLineExtent(startX,endX,layout.candles,view.srLineExtension);
  ...
  if (view.fibonacci) {
    ...
    fibs.forEach(({ fib, y }) => {
      ...
      const extent=calculateHorizontalLineExtent(startX,endX,layout.candles,view.fibLineExtension);
      context.beginPath();
  ...
  if(view.channels&&analysis.activeChannel){
    ...
    const basisLine=basis&&extendLineToPlot(basis,layout.candles,view.lrChannelExtension),upperLine=upper&&extendLineToPlot(upper,layout.candles,view.lrChannelExtension),lowerLine=lower&&extendLineToPlot(lower,layout.candles,view.lrChannelExtension);
          ...
          <div className="panel-scroll">
            {activePanel === "visuals" ? (
              <>
                ...
                {visualTab === "lines" ? <div className="setting-section"><h3>Pivot trendlines</h3><p className="field-help">Line extension reaches the visible plot edge and updates when the chart is panned or zoomed.</p>
