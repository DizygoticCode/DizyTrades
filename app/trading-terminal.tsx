...
import { coordinateJournalReplayLaunch } from "./lib/journal-trade-import";
...
export default function TradingTerminal({ user }: { user: AuthUser }) {
  ...
  useEffect(()=>{const controller=new AbortController();const timer=window.setTimeout(()=>{if(replayLaunchHandled.current)return;const query=new URLSearchParams(window.location.search),requestedMarketKey=query.get("replayMarketKey"),requestedSymbol=query.get("replaySymbol"),requestedTimeframe=query.get("replayTimeframe") as CandleTimeframe|null,at=Number(query.get("replayAt")),memoryId=query.get("replayMemory"),entryId=query.get("journalEntry");if(!requestedMarketKey&&!requestedSymbol&&!requestedTimeframe)return;if(!validJournalMarketKey(requestedMarketKey)||!requestedSymbol||!requestedTimeframe||!Number.isFinite(at)){replayLaunchHandled.current=true;setJournalReplayNotice("Replay data is unavailable for this trade.");return;}if(selectedMarketKey!==requestedMarketKey||symbol!==requestedSymbol||timeframe!==requestedTimeframe){setSelectedMarketKey(requestedMarketKey);setSymbol(requestedSymbol);setTimeframe(requestedTimeframe);return;}if(resultMarketKey!==marketKey)return;const rolling=Object.freeze(closedCandles.map(c=>Object.freeze({...c})));replayLaunchHandled.current=true;const clearQuery=()=>{for(const key of ["replayMarketKey","replaySymbol","replayTimeframe","replayAt","replayMemory","journalEntry"])query.delete(key);window.history.replaceState(null,"",`${window.location.pathname}${query.size?`?${query}`:""}`);};void coordinateJournalReplayLaunch({signal:controller.signal,request:{marketKey:requestedMarketKey,symbol:requestedSymbol,timeframe:requestedTimeframe,timestampMs:at},identity:{marketKey:requestedMarketKey,symbol:requestedSymbol,timeframe:requestedTimeframe},rollingCandles:rolling,loadRetained:memoryId&&entryId?async()=>{setJournalReplayNotice("Loading retained replay memory…");const response=await fetch(`/api/replay-memory/${encodeURIComponent(memoryId)}?journalEntry=${encodeURIComponent(entryId)}`,{signal:controller.signal});if(!response.ok)throw new Error("Retained memory unavailable");const {memory}=await response.json();if(memory.marketKey!==requestedMarketKey||memory.symbol!==requestedSymbol||memory.timeframe!==requestedTimeframe)throw new Error("Retained memory identity mismatch");return prepareReplayCandles(memory.candles,{symbol:requestedSymbol,timeframe:requestedTimeframe},{symbol:memory.symbol,timeframe:memory.timeframe});}:undefined}).then(result=>{if(result.source==="cancelled")return;clearQuery();if(result.source==="unavailable"){setJournalReplayNotice("Replay data is unavailable for this trade.");return;}const range=replayRangeForCandles(result.candles,requestedTimeframe),session=createReplaySession({id:`journal-${result.source}-${Date.now()}`,symbol:requestedSymbol,timeframe:requestedTimeframe,...range,startedAt:Date.now(),candles:result.candles,speed:1});setReplayCandles(result.candles);setReplaySession(jumpReplay(session,result.candles,result.cursor));setJournalReplayNotice(result.source==="retained-memory"?"Replay source: Retained memory. Historical DizyFlow data was not retained for this trade.":memoryId?"Replay source: Current history. Retained memory was unavailable.":"Replay source: Current history.");setViewportReset(v=>v+1);});},0);return()=>{window.clearTimeout(timer);controller.abort();};},[selectedMarketKey,symbol,timeframe,closedCandles,resultMarketKey,marketKey]);
