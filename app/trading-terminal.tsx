...
import { createReplaySession, createReplaySnapshot, jumpReplay, prepareReplayCandles, progressReplay, replayDelayMs, replayIdentityChanged, replayPrefix, replayRangeForCandles, stepReplay, type ReplaySession, type ReplaySpeed } from "./lib/replay";
...
export default function TradingTerminal({ user }: { user: AuthUser }) {
  ...
  useEffect(()=>{const query=new URLSearchParams(window.location.search),memoryId=query.get("replayMemory"),entryId=query.get("journalEntry");if(!memoryId||!entryId||replayLaunchHandled.current)return;const controller=new AbortController();void (async()=>{try{setJournalReplayNotice("Loading retained replay memory…");const response=await fetch(`/api/replay-memory/${encodeURIComponent(memoryId)}?journalEntry=${encodeURIComponent(entryId)}`,{signal:controller.signal});if(!response.ok)throw new Error();const {memory}=await response.json();const candles=prepareReplayCandles(memory.candles,{symbol:memory.symbol,timeframe:memory.timeframe},{symbol:memory.symbol,timeframe:memory.timeframe});if(memory.marketKey!==query.get("replayMarketKey")||memory.symbol!==query.get("replaySymbol")||memory.timeframe!==query.get("replayTimeframe"))throw new Error();const cursor=journalReplayCursor({marketKey:memory.marketKey,symbol:memory.symbol,timeframe:memory.timeframe,timestampMs:Number(query.get("replayAt"))},{marketKey:memory.marketKey,symbol:memory.symbol,timeframe:memory.timeframe,candles});if(cursor===null)throw new Error();replayLaunchHandled.current=true;setSelectedMarketKey(memory.marketKey);setSymbol(memory.symbol);setTimeframe(memory.timeframe);const session=createReplaySession({id:`retained-${memory.id}`,symbol:memory.symbol,timeframe:memory.timeframe,...replayRangeForCandles(candles,memory.timeframe),startedAt:Date.now(),candles,speed:1});setReplayCandles(candles);setReplaySession(jumpReplay(session,candles,cursor));setJournalReplayNotice("Replay source: Retained memory. Historical DizyFlow data was not retained for this trade.");for(const key of ["replayMarketKey","replaySymbol","replayTimeframe","replayAt","replayMemory","journalEntry"])query.delete(key);window.history.replaceState(null,"",`${window.location.pathname}${query.size?`?${query}`:""}`);setViewportReset(v=>v+1);}catch(reason){if((reason as Error).name!=="AbortError"){query.delete("replayMemory");query.delete("journalEntry");window.history.replaceState(null,"",`${window.location.pathname}?${query}`);setJournalReplayNotice("Retained memory is unavailable; checking current history.");}}})();return()=>controller.abort();},[]);
  useEffect(()=>{if(replayLaunchHandled.current)return;const query=new URLSearchParams(window.location.search);if(query.get("replayMemory"))return;const requestedMarketKey=query.get("replayMarketKey"),requestedSymbol=query.get("replaySymbol"),requestedTimeframe=query.get("replayTimeframe") as CandleTimeframe|null,at=Number(query.get("replayAt"));if(!requestedMarketKey&&!requestedSymbol&&!requestedTimeframe)return;const clearQuery=()=>{for(const key of ["replayMarketKey","replaySymbol","replayTimeframe","replayAt"])query.delete(key);window.history.replaceState(null,"",`${window.location.pathname}${query.size?`?${query}`:""}`);};const timer=window.setTimeout(()=>{if(!validJournalMarketKey(requestedMarketKey)||!requestedSymbol||!requestedTimeframe||!Number.isFinite(at)){replayLaunchHandled.current=true;clearQuery();setJournalReplayNotice("Replay data is unavailable for this trade.");return;}if(selectedMarketKey!==requestedMarketKey||symbol!==requestedSymbol||timeframe!==requestedTimeframe){setSelectedMarketKey(requestedMarketKey);setSymbol(requestedSymbol);setTimeframe(requestedTimeframe);return;}if(!closedCandles.length||resultMarketKey!==marketKey)return;const candles=Object.freeze(closedCandles.map(c=>Object.freeze({...c}))),cursor=journalReplayCursor({marketKey:requestedMarketKey,symbol:requestedSymbol,timeframe:requestedTimeframe,timestampMs:at},{marketKey:selectedMarketKey,symbol,timeframe:timeframe as CandleTimeframe,candles});replayLaunchHandled.current=true;clearQuery();if(cursor===null){setJournalReplayNotice("Replay data is unavailable for this trade.");return;}const range=replayRangeForCandles(candles,timeframe as CandleTimeframe),session=createReplaySession({id:`journal-replay-${Date.now()}`,symbol,timeframe:timeframe as CandleTimeframe,...range,startedAt:Date.now(),candles,speed:1});setJournalReplayNotice("");setReplayCandles(candles);setReplaySession(jumpReplay(session,candles,cursor));setViewportReset(v=>v+1);},0);return()=>window.clearTimeout(timer);},[selectedMarketKey,symbol,timeframe,closedCandles,resultMarketKey,marketKey]);
