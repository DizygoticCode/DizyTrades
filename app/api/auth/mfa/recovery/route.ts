...
import { consumeRateLimit, regenerateRecoveryCodes, verifyAccountPassword, verifyCurrentMfa } from "../../../../lib/auth-db";
...
export async function POST(request: Request) {
  ...
  if (consumeRateLimit([`mfa:recovery:user:${user.id}`, `mfa:recovery:ip:${requestIp(request)}`], 5, 15 * 60_000)) return NextResponse.json({ error: "Too many re-authentication attempts." }, { status: 429 });
  ...
  const recoveryCodes = regenerateRecoveryCodes(user.id); await appendAudit(user.id, "auth.mfa-recovery-regenerated", { sessionsRevoked: true }); return NextResponse.json({ recoveryCodes });
}
