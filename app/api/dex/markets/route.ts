...
export async function GET(request:Request){
  ...
  catch(error){return NextResponse.json({markets:[],provider:documentedDexProvider.id,degraded:error instanceof Error?error.message:"Provider unavailable"},{status:503});}
}
