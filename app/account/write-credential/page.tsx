...
export default async function OwnerWriteCredentialProvisioningPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return (
    <main className={accountStyles.page}>
      <header className={accountStyles.header}>
        <div>
          ...
          <p className={accountStyles.intro}>
            Bind one server-owned account/generation to the proven production host /32, attest a dedicated MEXC Order Placing key, and immediately seal it into encrypted custody. Successful completion ends at <strong>attested</strong>, never active.
          </p>
        </div>
      ...
      {result === "declared" ? <section className={accountStyles.notice} role="status"><div><p className={accountStyles.eyebrow}>EGRESS DECLARED</p><h2>The exact server-owned generation now has a declared production host /32.</h2><p>Use a new owner proof for each observation.</p></div></section> : null}
      ...
      {!identity ? (
        ...
        <>
          <section className={accountStyles.statusGrid} aria-label="Write credential ceremony status">
            ...
            <article className={accountStyles.statusCard}><span>Production host /32 proof</span><strong data-status={state?.status === "allowlisted" ? "fresh" : undefined}>{state?.status ?? "Unavailable"}</strong><small>{state?.dedicatedIpv4s[0] ? `${state.dedicatedIpv4s[0]}/32 · ${state.observationCount} observations` : "Not declared"}</small></article>
