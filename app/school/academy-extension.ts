...
const enhancements: Record<string, Section[]> = {
  ...
  dizyflow: [
    { heading: "The DizyFlow system", paragraphs: ["DizyFlow combines public market-microstructure views. The DOM shows current resting depth, trade bubbles summarise public transactions, and retained-liquidity storage plus diagnostics can preserve observed depth through time. The Bookmap-style customer heatmap remains unresolved and is not reliably visible, so diagnostics must not be mistaken for a completed display."], bullets: ["DOM: where liquidity is advertised now.", "Heatmap: where meaningful liquidity persisted, moved or disappeared.", "Trade bubbles: where transactions actually occurred.", "Whale alerts: unusual size, not guaranteed informed activity."] },
    { heading: "How the liquidity heatmap works", paragraphs: ["Conceptually, an enabled heatmap records order-book snapshots and projects relative intensity by price and time. Brighter bands would represent greater observed resting size within the current scaling, but this describes the retained data and intended interpretation—not a guarantee that the current customer-facing heatmap renders reliably."], bullets: ["Brightness is relative to the selected symbol, window and scale.", "A visible wall can be cancelled before price reaches it.", "Price may trade through a wall if aggressive flow overwhelms it.", "Historical heatmap data requires continuous collection; it cannot be reconstructed perfectly from candles."] },
...
const advancedLessons: AcademyLesson[] = [
  ...
  { slug: "heatmap-liquidity", title: "Liquidity heatmaps: intended interpretation and current limits", group: "Advanced Order Flow", summary: "Interpret resting-liquidity history while respecting cancellation and spoofing risk.", diagram: "flow", sections: [
  ...
  { slug: "delta-footprints", title: "Future concept: delta, cumulative delta and footprints", group: "Advanced Order Flow", summary: "Learn the intended analytics without implying that finished footprint and delta tools are shipped.", sections: [
  ...
  { slug: "absorption-exhaustion", title: "Future concept: absorption, exhaustion and trapped traders", group: "Advanced Order Flow", summary: "Recognise when aggressive effort fails to produce the expected result.", sections: [
