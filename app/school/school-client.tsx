...
const publicAcademyLessons = academyLessons.filter((lesson) => lesson.slug !== "dizyops-diagnostics");

export default function SchoolClient() {
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState(publicAcademyLessons[0].slug);
  const [completed, setCompleted] = useState<string[]>([]);
  const [ready, setReady] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const [glossaryQuery, setGlossaryQuery] = useState("");
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => { setCompleted(readAcademyProgress(window.localStorage).filter((slug) => slug !== "dizyops-diagnostics")); setReady(true); });
    return () => window.cancelAnimationFrame(frame);
  }, []);
  useEffect(() => { if (ready) writeAcademyProgress(window.localStorage, completed); }, [completed, ready]);
  const visible = useMemo(() => filterAcademyLessons(query).filter((item) => item.slug !== "dizyops-diagnostics"), [query]);
  const lesson = publicAcademyLessons.find((item) => item.slug === selected) ?? publicAcademyLessons[0];
  const index = publicAcademyLessons.indexOf(lesson);
  ...
  return <main className="school-shell">
    ...
    <div className="school-mobile-bar"><button aria-expanded={menuOpen} aria-controls="course-navigation" onClick={() => setMenuOpen(!menuOpen)}>☰ Course contents</button><span>{completed.length}/{publicAcademyLessons.length} complete</span></div>
    <div className="school-layout">
      <aside className={menuOpen ? "school-sidebar open" : "school-sidebar"} id="course-navigation" aria-label="Course navigation">
        ...
        <div className="progress-copy"><span>Course progress</span><b>{Math.round(completed.length / publicAcademyLessons.length * 100)}%</b></div><progress max={publicAcademyLessons.length} value={completed.length}>{completed.length} of {publicAcademyLessons.length}</progress>
        {academyLessonGroups.map((group) => <section className="course-group" key={group}><h2>{group}</h2><ol>{visible.filter((item) => item.group === group).map((item) => <li key={item.slug}><button className={item.slug === lesson.slug ? "active" : ""} aria-current={item.slug === lesson.slug ? "page" : undefined} onClick={() => choose(item.slug)}><span className={completed.includes(item.slug) ? "lesson-check done" : "lesson-check"} aria-hidden="true">{completed.includes(item.slug) ? "✓" : publicAcademyLessons.indexOf(item) + 1}</span><span>{item.title}</span></button></li>)}</ol></section>)}
      ...
      <article className="lesson" id="lesson" tabIndex={-1} aria-label={`${SCHOOL_DISPLAY_NAME} lesson: ${lesson.title}`}>
        <div className="lesson-kicker"><span>{SCHOOL_DISPLAY_NAME} · {lesson.group}</span><span>Lesson {index + 1} of {publicAcademyLessons.length}</span></div><h1>{lesson.title}</h1><p className="lesson-lead">{lesson.summary}</p>
        ...
        <nav className="lesson-pager" aria-label="Lesson pagination">{index > 0 ? <button onClick={() => choose(publicAcademyLessons[index - 1].slug)}>← <span>Previous<br/><b>{publicAcademyLessons[index - 1].title}</b></span></button> : <span/>}{index < publicAcademyLessons.length - 1 ? <button onClick={() => choose(publicAcademyLessons[index + 1].slug)}><span>Next<br/><b>{publicAcademyLessons[index + 1].title}</b></span> →</button> : null}</nav>
