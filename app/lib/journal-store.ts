...
export function migrateJournalEntry(value:unknown):JournalEntry|null {
  ...
  if(entry.schemaVersion!==1&&entry.schemaVersion!==2&&entry.schemaVersion!==3&&entry.schemaVersion!==4&&entry.schemaVersion!==JOURNAL_SCHEMA_VERSION)return null;
  ...
  const flow=trade?.historicalDizyFlow&&typeof trade.historicalDizyFlow==="object"?trade.historicalDizyFlow as HistoricalDizyFlowReference:unavailableHistoricalDizyFlowReference();
...
export async function setJournalHistoricalDizyFlowReferenceWithinTransaction(userId:string,id:string,reference:HistoricalDizyFlowReference){const record=await readJournal(userId),index=record.entries.findIndex(e=>e.id===id);if(index<0||!record.entries[index].trade)return null;const current=record.entries[index],entry=Object.freeze({...current,trade:Object.freeze({...current.trade!,historicalDizyFlow:reference})}) as JournalEntry;record.entries[index]=entry;await writeJournal(userId,record);return entry;}
