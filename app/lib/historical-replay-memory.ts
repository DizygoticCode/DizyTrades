...
export type ReplayMemoryCaptureProvenance = Readonly<{
  source:"terminal-closed-candle-buffer";serverValidated:true;validationVersion:2;capturedAt:string;
...
export function validateHistoricalReplayMemory(value:unknown):HistoricalReplayMemory {
  ...
  if(v.schemaVersion!==1||v.source!=="paper-trade"||v.id!==replayMemoryId(v)||v.captureProvenance?.source!=="terminal-closed-candle-buffer"||v.captureProvenance.serverValidated!==true||v.captureProvenance.validationVersion!==2||v.createdAt!==v.captureProvenance.capturedAt)fail("MALFORMED_MEMORY","Malformed replay-memory provenance.");
