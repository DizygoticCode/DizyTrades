...
export const documentedDexProvider: DexProvider = {
  ...
  async discover({query="",chain,cursor},signal){
    ...
    const key=`${query}|${chain}|${page}`, hit=cache.get(key); if(hit)return hit;
    ...
    const url=query ? new URL("/latest/dex/search",DEXSCREENER_URL) : new URL(`/networks/${chain === "bsc" ? "bsc" : "solana"}/pools`,GECKO_URL);
    ...
    const result={markets:normaliseDexScreener(shaped).filter((m)=>!chain||m.chain===chain),nextCursor:query?undefined:String(page+1),provider:this.id}; cache.set(key,result); return result;
  },
  async candles({chain,poolAddress,interval,limit},signal){
    ...
    const aggregate=Math.max(1,parseInt(interval)); const url=new URL(`/networks/${chain === "bsc" ? "bsc" : "solana"}/pools/${encodeURIComponent(poolAddress)}/ohlcv/${timeframe}`,GECKO_URL);
