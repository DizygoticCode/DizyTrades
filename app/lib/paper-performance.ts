...
export function livePaperSnapshot(confirmed: BacktestSummary, mark: number | null, enabled: boolean): LivePaperSnapshot {
  ...
  if (!enabled || !open || mark == null || ![mark, open.entry, open.realisedPnl, open.remainingQuantity, open.pnl, confirmed.endingEquity, confirmed.initialEquity].every(Number.isFinite)) return { ...confirmed, pnl: confirmed.endingEquity - confirmed.initialEquity, openTrade: open, liveMtm: false };
