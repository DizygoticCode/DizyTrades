...
export class SqliteMexcWriteCredentialCustody {
  ...
  private assertBacking() {
    if (this.poisoned) return fail(); if (this.path === ":memory:") return;
  ...
  private db() {
    ...
    try {
      ...
      if (userVersion === 0) db.exec(`BEGIN IMMEDIATE;
        CREATE TABLE mexc_write_credential_custody(
          schema_version INTEGER NOT NULL CHECK(schema_version=1), user_id TEXT NOT NULL, account_id TEXT NOT NULL, write_generation TEXT NOT NULL,
          revision INTEGER NOT NULL CHECK(revision>=1), status TEXT NOT NULL CHECK(status IN ('sealed','revoked')),
          credential_fingerprint_sha256 TEXT NOT NULL CHECK(length(credential_fingerprint_sha256)=64), key_version INTEGER NOT NULL,
          nonce BLOB NOT NULL CHECK(length(nonce)=12), ciphertext BLOB NOT NULL, auth_tag BLOB NOT NULL CHECK(length(auth_tag)=16),
          egress_proof_revision INTEGER NOT NULL CHECK(egress_proof_revision>=1), egress_ip_set_digest_sha256 TEXT NOT NULL CHECK(length(egress_ip_set_digest_sha256)=64),
          egress_allowlisted_at TEXT NOT NULL CHECK(length(egress_allowlisted_at)<=64), created_at TEXT NOT NULL CHECK(length(created_at)<=64),
          updated_at TEXT NOT NULL CHECK(length(updated_at)<=64), revoked_at TEXT CHECK(revoked_at IS NULL OR length(revoked_at)<=64),
          PRIMARY KEY(user_id,account_id,write_generation));
        CREATE TABLE mexc_write_credential_custody_events(
          sequence INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, account_id TEXT NOT NULL, write_generation TEXT NOT NULL,
          revision INTEGER NOT NULL CHECK(revision>=1), kind TEXT NOT NULL CHECK(kind IN ('sealed','revoked')), occurred_at TEXT NOT NULL CHECK(length(occurred_at)<=64));
  ...
  private row(identity: MexcWriteCredentialIdentity) {
    return this.db().prepare("SELECT * FROM mexc_write_credential_custody WHERE user_id=? AND account_id=? AND write_generation=?")
      .get(identity.userId, identity.accountId, identity.writeCredentialGeneration) as Row | undefined;
  }
  read(identity: MexcWriteCredentialIdentity): MexcWriteCredentialCustodyReceipt | null {
    if (!validIdentity(identity)) return fail(); const row = this.row(identity); this.assertBacking(); return row ? receipt(row) : null;
  }
  seal(identity: MexcWriteCredentialIdentity, secret: MexcWriteCredentialSecret, evidence: MexcWriteCredentialEgressEvidence, at: string, expectedRevision = 0) {
    if (!validIdentity(identity) || expectedRevision !== 0 || !Number.isSafeInteger(evidence.revision) || evidence.revision < 1 ||
      !SHA256.test(evidence.ipSetDigestSha256) || !timestamp(evidence.allowlistedAt) || !timestamp(at)) return fail();
    ...
    const db = this.db(); try {
      ...
      db.prepare("INSERT INTO mexc_write_credential_custody_events(user_id,account_id,write_generation,revision,kind,occurred_at) VALUES(?,?,?,?,?,?)")
        .run(identity.userId, identity.accountId, identity.writeCredentialGeneration, 1, "sealed", at);
  ...
  revoke(identity: MexcWriteCredentialIdentity, at: string, expectedRevision: number) {
    if (!validIdentity(identity) || !timestamp(at) || !Number.isSafeInteger(expectedRevision) || expectedRevision < 1) return fail();
    const db = this.db(); try {
      db.exec("BEGIN IMMEDIATE"); const row = this.row(identity); if (!row) return fail(); const current = receipt(row);
      ...
      const changes = Number(db.prepare(`UPDATE mexc_write_credential_custody SET revision=?,status='revoked',updated_at=?,revoked_at=?
        WHERE user_id=? AND account_id=? AND write_generation=? AND revision=? AND status='sealed'`)
        .run(nextRevision, at, at, identity.userId, identity.accountId, identity.writeCredentialGeneration, expectedRevision).changes);
      if (changes !== 1) return fail();
      db.prepare("INSERT INTO mexc_write_credential_custody_events(user_id,account_id,write_generation,revision,kind,occurred_at) VALUES(?,?,?,?,?,?)")
        .run(identity.userId, identity.accountId, identity.writeCredentialGeneration, nextRevision, "revoked", at);
      db.exec("COMMIT"); this.harden(); this.assertBacking(); return receipt(this.row(identity)!);
    } catch { try { db.exec("ROLLBACK"); } catch {} return fail(); }
  }
  events(identity: MexcWriteCredentialIdentity) {
    if (!validIdentity(identity)) return fail();
    return Object.freeze((this.db().prepare("SELECT revision,kind,occurred_at FROM mexc_write_credential_custody_events WHERE user_id=? AND account_id=? AND write_generation=? ORDER BY sequence")
      .all(identity.userId, identity.accountId, identity.writeCredentialGeneration) as { revision: number | bigint; kind: string; occurred_at: string }[])
      .map((row) => Object.freeze({ revision: Number(row.revision), kind: row.kind, occurredAt: row.occurred_at })));
  }
  close() { try { this.database?.close(); } finally { this.database = null; this.fileIdentity = null; } }
}

export function mexcWriteCredentialCustodyDatabasePathForTests() {
  return join(process.env.DATA_DIR || join(process.cwd(), ".data"), "mexc-write-credential-custody.sqlite");
}
