...
export function validateExecutionControlDocument(value: unknown): ExecutionControlDocument {
  ...
  const keys = ["schemaVersion", "revision", "armed", "globalDisabled", "disabledUserIds", "disabledAccountIds", "maintenance", "emergencyStop", "providerObservedAt", "providerValidForMs", "updatedAt"];
  ...
  if (row.schemaVersion !== EXECUTION_CONTROL_VERSION || !Number.isSafeInteger(row.revision) || (row.revision as number) < 1
    ...
    || !identitiesValid(row.disabledUserIds) || !identitiesValid(row.disabledAccountIds)
  ...
  return Object.freeze({ ...(row as ExecutionControlDocument), disabledUserIds: Object.freeze([...(row.disabledUserIds as string[])]), disabledAccountIds: Object.freeze([...(row.disabledAccountIds as string[])]) });
}
...
export const failClosedExecutionControl = (now = new Date()): ExecutionControlDocument => Object.freeze({
  ...
  disabledUserIds: Object.freeze([]), disabledAccountIds: Object.freeze([]), maintenance: false,
...
export class SqliteExecutionControlStore implements ExecutionControlStore {
  ...
  private db(): DatabaseSync {
    ...
    try {
      ...
      this.database = db; this.harden(); this.read(); return db;
    } catch (error) { try { db?.close(); } catch {} this.database = null; if (error instanceof ExecutionControlStoreError) throw error; return unavailable(); }
  }
  read(): ExecutionControlDocument {
    try {
  ...
  switches(now = this.clock()): ExecutionKillSwitches {
    ...
    return Object.freeze({ armed: state.armed, globalDisabled: state.globalDisabled,
      disabledUserIds: new Set(state.disabledUserIds), disabledAccountIds: new Set(state.disabledAccountIds),
