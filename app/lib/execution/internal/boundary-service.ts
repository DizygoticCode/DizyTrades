...
import {
  executionKillSwitchReason,
  type ExecutionKillSwitches,
} from "./kill-switch";
...
import {
  ExecutionReconciliationStoreError,
...
const rejected = (reason: ExecutionRejectionCode): ExecutionBoundaryResponse =>
  Object.freeze({
...
const isAuthenticatedCaller = (
  value: unknown,
): value is AuthenticatedExecutionCaller => {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Partial<AuthenticatedExecutionCaller>;
  return (
    isNonEmptyString(candidate.callerId) &&
...
const isKillSwitchState = (value: unknown): value is ExecutionKillSwitches => {
  ...
  return (
    typeof candidate.globalDisabled === "boolean" &&
...
export class InternalExecutionBoundary {
  ...
  preview(request: ExecutionBoundaryRequest): ExecutionBoundaryResponse {
    ...
    try {
      const authenticated = this.dependencies.authenticateInternalCaller(
        request.callerAssertion,
      );
      if (authenticated === null) return rejected("CALLER_UNAUTHENTICATED");
      if (!isAuthenticatedCaller(authenticated))
    ...
    if (this.dependencies.executionOwnershipStore) {
      try {
        const ownership =
          this.dependencies.executionOwnershipStore.read(caller);
        const proofMs =
          ownership.proofObservedAt === null
            ? NaN
            : Date.parse(ownership.proofObservedAt);
        const proofAge =
          (this.dependencies.now?.() ?? new Date()).getTime() - proofMs;
        const staleProof =
    ...
    if (this.dependencies.executionReconciliationStore) {
      try {
        const reconciliation =
          this.dependencies.executionReconciliationStore.read(caller);
        const observedAt =
          reconciliation.observedAt === null
            ? NaN
            : Date.parse(reconciliation.observedAt);
        const age =
          (this.dependencies.now?.() ?? new Date()).getTime() - observedAt;
        const staleClean =
          reconciliation.status === "clean" &&
          (!Number.isFinite(age) ||
            age < 0 ||
            age > MEXC_PROVIDER_READBACK_MAX_AGE_MS);
        if (reconciliation.status !== "clean" || staleClean)
          killReason ??=
            reconciliation.status === "quarantined"
              ? "EXECUTION_ACCOUNT_QUARANTINED"
              : "EXECUTION_RECONCILIATION_UNKNOWN";
      } catch (error) {
        killReason ??=
          error instanceof ExecutionReconciliationStoreError
            ? error.code
