...
export class FlowAggregator {
  ...
  captureBook(book:BookView,contractSize:number,timeMs:number,rangeBps=50){
    ...
    for(const [side,levels] of [["bid",book.bids],["ask",book.asks]] as const)for(const level of levels){if(level.price<low||level.price>high)continue;const price=Math.round(level.price/this.options.priceStep)*this.options.priceStep,cell=observation.get(price)??{timestampMs:timeBucket,price,bidNotional:0,askNotional:0,bidQuantity:0,askQuantity:0,mid,spread:ask-bid};cell[side==="bid"?"bidNotional":"askNotional"]+=level.price*level.contractQuantity*contractSize;cell[side==="bid"?"bidQuantity":"askQuantity"]+=level.contractQuantity;observation.set(price,cell);}
    ...
    for(const cell of observation.values())this.heatmapBuckets.set(`${cell.timestampMs}:${cell.price}`,cell);
    this.prune(timeMs);
  }
  ...
  private prune(now:number){const cutoff=now-this.options.historyMs;for(const [key,value] of this.heatmapBuckets)if(value.timestampMs<cutoff)this.heatmapBuckets.delete(key);while(this.heatmapBuckets.size>this.options.maxCells)this.heatmapBuckets.delete(this.heatmapBuckets.keys().next().value!);while(this.bubbleBuckets.size>this.options.maxBubbles)this.bubbleBuckets.delete(this.bubbleBuckets.keys().next().value!);this.heatmap=[...this.heatmapBuckets.values()];this.bubbles=[...this.bubbleBuckets.values()];}
}
export type LiquiditySegment={price:number;side:"bid"|"ask";startMs:number;endMs:number;quantity:number};
export function buildLiquiditySegments(cells:readonly HeatmapCell[],expectedIntervalMs=1_000){const groups=new Map<string,HeatmapCell[]>();for(const cell of cells)for(const side of ["bid","ask"] as const){const quantity=side==="bid"?cell.bidQuantity:cell.askQuantity;if(quantity>0){const key=`${cell.price}:${side}`,values=groups.get(key)??[];values.push(cell);groups.set(key,values)}}const result:LiquiditySegment[]=[];for(const [key,values] of groups){values.sort((a,b)=>a.timestampMs-b.timestampMs);const side=key.endsWith(":bid")?"bid" as const:"ask" as const;let start=values[0],previous=values[0],quantity=side==="bid"?start.bidQuantity:start.askQuantity;for(const value of values.slice(1)){if(value.timestampMs-previous.timestampMs>expectedIntervalMs*1.5){result.push({price:start.price,side,startMs:start.timestampMs,endMs:previous.timestampMs+expectedIntervalMs,quantity});start=value;quantity=0}quantity=Math.max(quantity,side==="bid"?value.bidQuantity:value.askQuantity);previous=value}result.push({price:start.price,side,startMs:start.timestampMs,endMs:previous.timestampMs+expectedIntervalMs,quantity})}return result}
