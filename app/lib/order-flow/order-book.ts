...
export class OrderBook {
  ...
  private drain() { while (this.future.has(this._version + 1)) { const next = this.future.get(this._version + 1)!; this.future.delete(next.version); this.applyLevels(next); this._version = next.version; } this._valid = this.future.size === 0 || Math.min(...this.future.keys()) === this._version + 1; }
