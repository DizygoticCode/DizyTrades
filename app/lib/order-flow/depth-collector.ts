...
export class DepthCollector{
 private timer:ReturnType<typeof setTimeout>|null=null;private inFlight=false;private failures=0;private error:string|null=null;private latest:DepthSnapshot|null=null;private history:DepthSnapshot[]=[];private emitter=new EventEmitter();private running=false;private connectionState="stopped";
 ...
 async poll(){if(this.inFlight)return false;this.inFlight=true;let delay=POLL_MS;try{const response=await this.fetcher(`${REST_BASE}/api/v1/contract/depth/${encodeURIComponent(this.symbol)}?limit=100`,{cache:"no-store",signal:AbortSignal.timeout(TIMEOUT_MS),headers:{accept:"application/json"}});if(!response.ok)throw Error(`MEXC depth HTTP ${response.status}`);const snapshot=normalizeMexcSnapshot(await response.json(),this.symbol);this.latest=snapshot;this.failures=0;this.error=null;this.transition("live");this.history.push(snapshot);const cutoff=this.now()-HISTORY_MS;while(this.history.length>MAX_HISTORY||this.history[0]?.engineTimeMs<cutoff)this.history.shift();this.emitter.emit("snapshot",snapshot);}catch(error){this.failures++;this.error=safeError(error);this.transition(this.latest?"degraded":"error");delay=Math.min(30_000,POLL_MS*2**Math.min(this.failures,5));}finally{this.inFlight=false;if(this.running)this.timer=setTimeout(()=>void this.poll(),delay);}return true;}
 ...
 diagnostic():CollectorDiagnostic{return{symbol:this.symbol,running:this.running,lastSuccessfulSnapshot:this.latest?.engineTimeMs??null,snapshotAgeMs:this.latest?Math.max(0,this.now()-this.latest.engineTimeMs):null,lastVersion:this.latest?.version??null,bids:this.latest?.bids.length??0,asks:this.latest?.asks.length??0,consecutiveFailures:this.failures,lastError:this.error,subscribers:this.emitter.listenerCount("snapshot")};}
}
