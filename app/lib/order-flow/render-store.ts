...
type Listener=()=>void;
type HeatmapRenderSettings=OrderFlowSettings["heatmap"]&{renderVisible:boolean};
...
export class FlowRenderStore {
  ...
  private viewportListeners=new Set<(range:LiquidityViewport)=>void>();private lastViewport:LiquidityViewport|null=null;private lastViewportGeneration="";
  ...
  updateSettings(settings:OrderFlowSettings){
    const heatmap={...settings.heatmap,renderVisible:settings.heatmapVisible} as HeatmapRenderSettings;
    this.update({settings:{...settings,heatmap}});
  }
