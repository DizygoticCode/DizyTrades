...
export type FlowRenderSnapshot={generation:string;enabled:boolean;bookValid:boolean;captureStarted:number|null;heatmap:readonly HeatmapCell[];bubbles:readonly VolumeBubble[];book:BookView;settings:OrderFlowSettings};
type Listener=()=>void;
...
export class FlowRenderStore {
  ...
  constructor(settings:OrderFlowSettings){this.snapshot={generation:"",enabled:false,bookValid:false,captureStarted:null,heatmap:[],bubbles:[],book:emptyBook,settings};}
  getSnapshot=()=>this.snapshot;
  subscribe=(listener:Listener)=>{this.listeners.add(listener);return()=>{this.listeners.delete(listener)}};
  update(next:Partial<FlowRenderSnapshot>){this.snapshot={...this.snapshot,...next};this.requestNotification();}
  reset(generation:string,settings=this.snapshot.settings){this.snapshot={generation,enabled:settings.enabled,bookValid:false,captureStarted:settings.enabled?Date.now():null,heatmap:[],bubbles:[],book:emptyBook,settings};this.requestNotification();}
