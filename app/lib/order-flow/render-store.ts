...
export class FlowRenderStore {
  ...
  private viewportListeners=new Set<(range:LiquidityViewport)=>void>();private lastViewport:LiquidityViewport|null=null;
  private snapshot:FlowRenderSnapshot;private diagnostics:FlowRenderDiagnostics;
  constructor(settings:OrderFlowSettings){this.snapshot={generation:"",enabled:false,bookValid:false,captureStarted:null,captureEnded:null,priceStep:1,heatmap:[],heatmapTiles:[],tileTimeBucketMs:5000,trades:[],book:emptyBook,settings};this.diagnostics=initialDiagnostics(settings);}
  ...
  requestHistory(range:LiquidityViewport){if(this.lastViewport&&Math.abs(this.lastViewport.from-range.from)<1000&&Math.abs(this.lastViewport.to-range.to)<1000&&Math.abs(this.lastViewport.minPrice-range.minPrice)<range.effectivePriceStep)return;this.lastViewport=range;this.viewportListeners.forEach(listener=>listener(range));}
