...
async function refresh(signal?: AbortSignal) {
  const [spotInfo,futureInfo] = await Promise.all([fetch("https://api.mexc.com/api/v3/exchangeInfo",{signal,cache:"no-store"}),fetch("https://contract.mexc.com/api/v1/contract/detail",{signal,cache:"no-store"})]);
