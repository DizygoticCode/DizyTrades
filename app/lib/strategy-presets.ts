export type StrategyMode = "scalp-15m" | "swing-1h-4h" | "custom";
...
export const strategyModeLabel=(mode:StrategyMode)=>mode==="scalp-15m"?"Scalping · 15m":mode==="swing-1h-4h"?"Swing · 1H/4H":"Custom";
...
export const strategyHistoryCapacity=(settings:StrategySettings)=>settings.mode==="swing-1h-4h"?1400:settings.mode==="scalp-15m"?800:Math.min(2000,Math.max(800,settings.srLookback+Math.max(settings.channelLength,settings.fibLength)+settings.pivotLength*2));
