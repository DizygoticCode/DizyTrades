...
export function reselectPaperRiskTier(snapshot:PaperRiskTierSnapshot,exposure:number):PaperRiskTierSnapshot{
 ...
 if(snapshot.calculationMethod==="contract-tier-increments"){
  ...
  return Object.freeze({...snapshot,level,exposure,maxExposure:(base as number)+(increment as number)*(level-1),maintenanceMarginRate,initialMarginRate,maxLeverage:maxLeverageFor(snapshot.contractMaxLeverage,initialMarginRate)});
 }
